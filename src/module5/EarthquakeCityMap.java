...
public class EarthquakeCityMap extends PApplet {
	...
	private void hideMarkers(Marker m) {
		double rad = 0;
		rad = ((EarthquakeMarker) m).threatCircle() *3;
		for(Marker marker : quakeMarkers) {
			marker.setHidden(true);
		}
			
		for(Marker marker : cityMarkers) {
			if(m.getDistanceTo(marker.getLocation()) > rad) {
				marker.setHidden(true);
			}
		}
		m.setHidden(false);	
	...
	private void hideMarkers2(Marker m) {
		for(Marker marker : quakeMarkers) {
			if(m.getDistanceTo(marker.getLocation()) > ((EarthquakeMarker) marker).threatCircle()*3) {
				marker.setHidden(true);
			}
		}
